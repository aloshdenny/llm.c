...
void gpt2_init_random(GPT2 *model, int max_seq_len, int vocab_size, int num_layers, int num_heads, int channels) {
    ...
    // Using scaled initialization: range approximately [-0.1, 0.1] for numerical stability
    uint64_t seed = 12345;
    float init_scale = 0.1f;
    for (size_t i = 0; i < num_parameters; i++) {
        // Simple LCG-style random number generator
        seed = seed * 6364136223846793005ULL + 1442695040888963407ULL;
        // Convert to float in range [0, 1]
        float rand_val = ((seed >> 32) & 0xFFFFFFFF) / (float)0xFFFFFFFF;
        // Scale to [-init_scale, init_scale]
        model->params_memory[i] = (rand_val * 2.0f - 1.0f) * init_scale;
    }
...
int main() {
    ...
    // build the GPT-2 model from a checkpoint or initialize from scratch
    GPT2 model;
    const char* checkpoint_path = "gpt2_124M.bin";
    
    // Check if checkpoint exists, otherwise initialize from scratch
    if (access(checkpoint_path, F_OK) != -1) {
        printf("Loading model from checkpoint: %s\n", checkpoint_path);
        gpt2_build_from_checkpoint(&model, checkpoint_path);
    } else {
        printf("Checkpoint not found, initializing model from scratch\n");
        ...
        int num_heads = 12;
        int channels = 768;
        gpt2_init_random(&model, max_seq_len, vocab_size, num_layers, num_heads, channels);
    }
