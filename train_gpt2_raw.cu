...
int main(int argc, char *argv[]) {
    ...
    const char* default_load_filename = "gpt2_124M_bf16.bin"; // bf16 weights of the model
    const char* load_filename = default_load_filename;
    ...
    if (resuming == 1) {
        ...
        // If the default checkpoint isn't present, fall back to random init via descriptor.
        // This keeps the program runnable out-of-the-box while still allowing `-e` to point
        // at a real checkpoint.
        if (strcmp(load_filename, default_load_filename) == 0 && access(load_filename, F_OK) != 0) {
            const char* fallback_descriptor = "d12"; // GPT-2 124M
            printf0("Checkpoint file not found: %s\n", load_filename);
            printf0("Falling back to random initialization with descriptor: %s\n", fallback_descriptor);
            load_filename = fallback_descriptor;
